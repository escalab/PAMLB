...
class VicunaAdapter(BaseModelAdapter):
    ...
    def load_model(self, model_path: str, from_pretrained_kwargs: dict):
        ...
        print(f"[PAMLBench] Tokenizer loading time {(time.time() - start_time):.6f}s")
        # print(f"[PAMLBench] Tokenizer loading (CPU exec) time {(time.process_time() - cpu_start_time):.6f}s")
        cpu_start_time = time.process_time()
        start_time = time.time()
        model = AutoModelForCausalLM.from_pretrained(
            model_path,
        ...
        print(f"[PAMLBench] Model loading time {(time.time() - start_time):.6f}s")
        # print(f"[PAMLBench] Model loading (CPU exec) time {(time.process_time() - cpu_start_time):.6f}s")
        return model, tokenizer
